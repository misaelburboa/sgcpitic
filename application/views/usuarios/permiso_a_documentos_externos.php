...
function activarActuales(){
	...
	if(puesto_id != ""){
		$.ajax({
			type:'POST',
			url:'<?php echo base_url()."getJobsCurrentDocsExt/"; ?>'+puesto_id,
			success: function(respuesta){
	            $(respuesta).each(function(i, v){ // indice, valor
	                $('#actuales').append('<option value="'+v.id_doc_externo+'" title="'+v.nombre_documento+'">'+v.nombre_documento+'</option>');
	            })
...
function activarDisponibles(){
	...
	if(puesto_id != ""){
		$.ajax({
			type:'POST',
			url:'<?php echo base_url()."getAvailableDocsExt/"; ?>'+puesto_id,
			success: function(respuesta){
	            // Limpiamos el select
	            $('#disponibles').find('option').remove();

	            $(respuesta).each(function(i, v){ // indice, valor
	                $('#disponibles').append('<option value="'+v.id_documento+'" title="'+v.nombre_documento+'">'+v.nombre_documento+'</option>');
	            })
    ...
    while(cont < selectedVals.length){
    	if(buscarEnSelect(selectedVals[cont].value, optDisp)==false){
	    	...
	    	while(j < optActuales.options.length){
				optActuales.options[j].value
	    		if(optActuales.options[j].value == selectedVals[cont].value){
	    			$.ajax({
						type:'POST',
						url:'<?php echo base_url()."removeDocumentAccessExt/"; ?>'+document.getElementById('puesto').value+'/'+optActuales.options[j].value,
						success: function(respuesta){
				            exito = true;
						},
    ...
    while(cont < selectedVals.length){
    	if(buscarEnSelect(selectedVals[cont].value, optActuales)==false){
	    	...
	    	$.ajax({
				type:'POST',
				async: true,
				url:'<?php echo base_url()."grantDocumentAccessExt/"; ?>'+document.getElementById('puesto').value+'/'+selectedVals[cont].value,
				success: function(respuesta){
		            exito = true;
				},
				failure: function(respuesta){
